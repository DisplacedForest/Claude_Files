# PowerShell script to spawn Claude lint fixer agent
...
$featureDir = "FOR_REVIEW_issue-$FeatureName"
$wslFeaturePath = "$wslPath/$featureDir"

# Build the command to run the lint fixer
$task = "/lint_fixer $FeatureName"
# Stay in project root, don't cd into feature directory
$command = "cd '$wslPath' && /home/zachary/.npm-global/bin/claude '$task'"

# Open new Windows Terminal window for lint fixer
wt.exe -w new wsl -d Ubuntu -- bash -c "$command"

Write-Host "Spawned Lint Fixer for feature: $FeatureName" -ForegroundColor Red
Write-Host "Working in: $featureDir" -ForegroundColor Yellow
Write-Host "Will fix linting and pre-commit hook errors" -ForegroundColor Cyan