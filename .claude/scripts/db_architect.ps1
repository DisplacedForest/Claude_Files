# PowerShell script to spawn Claude database architect agent
...
# Build the command to run the database architect
$task = "/db_architect $FeatureName"
# Stay in project root, don't cd into feature directory
$command = "cd '$wslPath' && /home/zachary/.npm-global/bin/claude '$task'"

# Open new Windows Terminal window for database architect
wt.exe -w new wsl -d Ubuntu -- bash -c "$command"

Write-Host "Spawned Database Architect for feature: $FeatureName" -ForegroundColor Blue
Write-Host "Working in: $featureDir" -ForegroundColor Yellow
Write-Host "Will read orchestrator.md and complete database tasks" -ForegroundColor Cyan